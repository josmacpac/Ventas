    ...
    Array.from(checkbox).forEach(e => {
      if (e.checked) {
        ...
        if (index != -1) {
          ...
          carrito.push({id_cliente:"", id_articulo: e.id, quantity: 1, nombre: " ", precio: 0 })
          console.log(carrito)
  ...
  for (const product of carrito) { // recorremos el carrito
    resolvedPromises = resolvedPromises.concat(new Promise((resolve, reject) => { // creamos el array de promesas
      ...
      fetch(`http://localhost:3000/productos/api/id/${product.id_articulo}`)
        ...
        .then(data => {
          let index = carrito.findIndex((prod) => prod.id_articulo == product.id_articulo)
          carrito[index].precio = data.venta
...
const actualizarTotal = () => {
  totalCarrito.innerHTML = ""
  let carrito = JSON.parse(localStorage.carrito);
  const total = (carrito.reduce((acc, curr) => acc += (curr.precio * curr.quantity), 0)).toFixed(2)
